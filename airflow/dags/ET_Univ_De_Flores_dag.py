...
Universidad Nacional De Flores
...
def sql_to_csv_data_flores():
    """ Obtener datos sql y convertirlos en datframe """
    sql_src = os.path.join(folder, 'include/universidad_de_flores.sql')
    ...
    cols = ['university', 'career', 'inscription_date', 'first_name', 'gender', 'age', 'postal_code', 'location', 'email']
    df.columns = cols
    df.to_csv(os.path.join(folder, 'files/ET_Univ_Nacional_Flores.csv'), header=cols, index=False)
...
def process_dataframe():
    ...
    today = datetime.now()
    df = pd.read_csv(os.path.join(folder, 'files/ET_Univ_Nacional_Flores.csv'))
...
with DAG(
    dag_id='python_operator_univ_flores',
    ...
    def create_csv():
        ...
        sql_to_csv_data_flores()
        log.info('CSV created')

    @task(task_id="dataframe_task")
    def dataframe_task():
        """ DAG que procesa el dataframe """
        process_dataframe()
        log.info(f'df created')
