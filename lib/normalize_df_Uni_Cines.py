...
def normalize_df_Uni_Cines(path_df, path_dfmerge, path_download):
  ...
  #df.columns = [(lambda x: x.lower())(x)for x in list(df.columns)]
  ...
  for row_i in range(len(df)):
    ...
    #### inscription_date
    df.loc[row_i, 'inscription_date'] = str(df.loc[row_i, 'inscription_date'])
  ...
  df = df.merge(dfmerge, on='location', how='left').drop(['full_name'], axis=1)
